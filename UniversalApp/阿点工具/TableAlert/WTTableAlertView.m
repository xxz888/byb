...
#define rowHeight  40   //定义tableView行高
...
- (UIButton *)selectionButton{
    ...
    if (!_selectionButton) {
        ...
        [_selectionButton setImage:[UIImage imageNamed:@"icon_duoxuan"] forState:UIControlStateSelected];
        [_selectionButton setImage:[UIImage imageNamed:@"icon_weiduoxuan"] forState:UIControlStateNormal];
        // 用代码添加约束时，该属性需要设置为no
...
+ (WTTableAlertView *_Nonnull)initWithTitle:(NSString *)title options:(NSArray *)optionsArray
    ...
    view.contentHConstraint.constant = view.tableviewHConstraint.constant + view.topViewHConstraint.constant + kBottomViewStyleNormalH - 40;
...
- (UILabel *)titleLabel{
    ...
    if (!_titleLabel) {
        ...
        _titleLabel.textColor = JDRGBAColor(51, 51, 51);
...
- (UIButton *)cancelBtn {
    if (!_cancelBtn) {
        ...
        [_cancelBtn setTitleColor: JDRGBAColor(0, 163, 255) forState:UIControlStateNormal];
        ...
        NSLayoutConstraint *bottom = [NSLayoutConstraint constraintWithItem:_cancelBtn attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:_bottomView attribute:NSLayoutAttributeBottom multiplier:1 constant:-5];
...
- (UIButton *)confirmBtn {
    if (!_confirmBtn) {
        ...
        [_confirmBtn setTitleColor: JDRGBAColor(0, 163, 255)
        ...
        NSLayoutConstraint *bottom = [NSLayoutConstraint constraintWithItem:_confirmBtn attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:_bottomView attribute:NSLayoutAttributeBottom multiplier:1 constant:-5];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell.selectionButton addTarget:self action:@selector(clickSelectionButton:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)clickSelectionButton:(UIButton *)btn{
    NSIndexPath * indexPath = [NSIndexPath indexPathForRow:btn.tag -100 inSection:0];
    WTSelectionCell *cell = (WTSelectionCell *)[self.tableView cellForRowAtIndexPath:indexPath];
    if (self.isSingleSelection) {
        ...
        if (self.selectedArray && self.selectedArray.count > 0) {
            ...
            WTSelectionCell *cell = (WTSelectionCell *)[self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:[lastRow integerValue] inSection:0]];
            cell.selectionButton.selected = NO;
        }
        [self.selectedArray removeAllObjects];
        [self.selectedArray addObject:@(indexPath.row)];
