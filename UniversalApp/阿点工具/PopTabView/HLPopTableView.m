...
//    self.backView.backgroundColor = [UIColor redColor];
    ...
    if (popFrame.origin.y + frame.size.height <= WINDOW.height - 30) {
        self.showView.frame = CGRectMake(popFrame.origin.x, popFrame.origin.y + popFrame.size.height , frame.size.width , frame.size.height+10);
    }
    if (popFrame.origin.y + frame.size.height > WINDOW.height - 30 ) {
        self.showView.frame = CGRectMake(popFrame.origin.x , popFrame.origin.y - frame.size.height, frame.size.width , frame.size.height+10);
    }
...
//    self.showView.backgroundColor = [UIColor greenColor];
    ...
    UITableView * popTable = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, self.showView.width, self.showView.height) style:0];
    popTable.separatorStyle = 0;
