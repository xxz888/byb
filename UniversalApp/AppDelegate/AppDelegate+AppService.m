...
//崩溃日志
-(void)initBugly{
    [Bugly startWithAppId:UPDATE_App_ID];
}
...
-(void)initUserManager{
    if ([kUserDefaults objectForKey:AUTO_LOGIN]) {
        [AD_MANAGER.userDataArray removeAllObjects];
        LoginModel * model = [[LoginModel alloc]init];
        [model setValuesForKeysWithDictionary:[kUserDefaults objectForKey:AUTO_LOGIN]];
        [AD_MANAGER.userDataArray addObject:model];
    ...
    [PPNetworkHelper networkStatusWithBlock:^(PPNetworkStatusType networkStatus) {
        ...
        switch (networkStatus) {
            ...
            case PPNetworkStatusUnknown:{
                [UIView showToast:@"网络环境：未知网络"];
            }
                // 无网络
            case PPNetworkStatusNotReachable:
                [UIView showToast:@"网络环境：无网络"];
                KPostNotification(KNotificationNetWorkStateChange, @NO);
                break;
                // 手机网络
            case PPNetworkStatusReachableViaWWAN:
                [UIView showToast:@"网络环境：手机自带网络"];
                // 无线网络
            case PPNetworkStatusReachableViaWiFi:
                [UIView showToast:@"网络环境：WiFi"];
