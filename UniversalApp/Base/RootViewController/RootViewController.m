...
-(void)showNoDataImage
{
    ...
    [_noDataView setImage:[UIImage imageNamed:@"default"]];
...
- (NSMutableArray<UIButton *> *)addNavigationItemWithTitles:(NSArray *)titles isLeft:(BOOL)isLeft target:(id)target action:(SEL)action tags:(NSArray *)tags
{
    ...
    for (NSString * title in titles) {
        UIButton * btn = [UIButton buttonWithType:UIButtonTypeCustom];
        btn.frame = CGRectMake(0, 0, 100, 21);
...
- (void)keyboardWillShow:(NSNotification *)notification{
    //    CGFloat h = self.btnRegister.frame.origin.y +self.btnRegister.frame.size.height;
    //获取键盘的高度
