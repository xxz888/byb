...
{
    //扫描完成
    if ([metadataObjects count] > 0 && !_iscome) {
        _iscome = YES;
        //显示结果
          ...
          NSString * value = [[metadataObjects firstObject] stringValue];
        
        
            NSMutableDictionary * mDic = [AD_SHARE_MANAGER requestSameParamtersDic:@{@"pageno":@"1",@"pagesize":@"2000",@"keywords":value,@"sort":@"",@"showdj":@1}];
        
        kWeakSelf(self);
        [AD_MANAGER requestSelectSpPage:mDic success:^(id str) {
            NSArray * spArray = str[@"data"][@"list"];
            if ([spArray count] != 0) {
                JDSelectSpModel  * model = [[JDSelectSpModel alloc]init];
                [model setValuesForKeysWithDictionary:spArray[0]];
                VC.spModel = model;
                VC.hidesBottomBarWhenPushed = YES;
                [self.navigationController pushViewController:VC animated:YES];
                [self stopScanning];
            }else{
                [UIView showToast:@"未识别条码,请手动输入"];
