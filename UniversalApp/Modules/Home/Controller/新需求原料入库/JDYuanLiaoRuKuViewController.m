//
//  JDYuanLiaoRuKuViewController.m
//  UniversalApp
//
...
#import "JDYuanLiaoRuKuViewController.h"
...
@interface JDYuanLiaoRuKuViewController ()<YBPopupMenuDelegate>
...
@implementation JDYuanLiaoRuKuViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"原料入库单确认";
}
...
-(void)allOrderCommonParamters{
    ...
    [AD_MANAGER.caoGaoDic setValue:@([AD_MANAGER.affrimDic[@"jsrid"] integerValue]) forKey:@"jsrid"];//经手人就是自己
    NSInteger jgcid = [AD_MANAGER.affrimDic[@"jgcidid"] integerValue];
    [AD_MANAGER.caoGaoDic setValue:@(jgcid) forKey:@"jgcid"];//供应商名称id
    [AD_MANAGER.caoGaoDic setValue:@([AD_MANAGER.affrimDic[@"zjfc"] integerValue]) forKey:@"zjfc"];//是否直发
...
-(void)yudingdanCommonAction:(NSInteger )autoCheck{
    ...
    [AD_MANAGER.caoGaoDic setValue:[self allOrderYangPinYuDingListCommonParamtersArray] forKey:@"tbnote_jhrkcbs"];//商品列表
        [AD_MANAGER.caoGaoDic setValue:@[] forKey:@"tbnote_jhrkcb_fks"];//商品列表
        [AD_MANAGER.caoGaoDic setValue:@[] forKey:@"tbnote_jhrkcb_zks"];//商品列表
    ...
    [AD_MANAGER requestYuanLiaoRuKuUp:mDic success:^(id object) {
        UIStoryboard * stroryBoard4 = [UIStoryboard storyboardWithName:@"ForthVC" bundle:nil];
        if (autoCheck == 1) {
            [self showToast:[@"原料入库单保存成功" append:object[@"data"]]];
