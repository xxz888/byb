//
//  JDJiaGongShouHuo5ViewController.m
//  UniversalApp
//
...
#import "JDYuanLiaoRuKuTableViewController.h"

@interface JDJiaGongShouHuo5ViewController ()<YBPopupMenuDelegate>
@property(nonatomic,strong)JDYuanLiaoRuKuTableViewController * VC;
...
@implementation JDJiaGongShouHuo5ViewController
...
-(NSMutableArray *)allOrderYangPinYuDingListCommonParamtersArray{
    ...
    NSMutableArray * colorModelArray = [[NSMutableArray alloc]init];
    for (NSInteger i = 0 ; i < AD_MANAGER.sectionArray.count; i++) {
        ...
        //查找颜色的model
        [colorModelArray addObject:countArr[0]];
    }
    JDAddColorModel * colorModel1 = colorModelArray[0];
    JDAddColorModel * colorModel2 = colorModelArray[1];
    NSMutableDictionary * spxscbsDic = [[NSMutableDictionary alloc]init];
    [spxscbsDic setValue:@(0) forKey:@"dnxh"];
    [spxscbsDic setValue:@(colorModel1.spid) forKey:@"spid"];//商品ID
    [spxscbsDic setValue:@(0) forKey:@"cdid"];
    [spxscbsDic setValue:colorModel1.sh forKey:@"sh"];//色号
    [spxscbsDic setValue:@"0" forKey:@"gh"];//色号
    [spxscbsDic setValue:@([colorModel1.savePishu integerValue]) forKey:@"spps"];//匹数
    [spxscbsDic setValue:@([colorModel1.saveCount doubleValue]) forKey:@"spsl"];//销售数量
    //坯布的
    [spxscbsDic setValue:@(colorModel2.spid) forKey:@"pbspid"];//商品ID
    [spxscbsDic setValue:colorModel2.sh forKey:@"pbsh"];//色号
    [spxscbsDic setValue:@([colorModel2.savePishu integerValue]) forKey:@"pbps"];//匹数
    [spxscbsDic setValue:@([colorModel2.saveCount doubleValue]) forKey:@"pbsl"];//销售数量
    
    //这两个不知道什么参数
    [spxscbsDic setValue:@(0) forKey:@"cpps"];
    [spxscbsDic setValue:@(0) forKey:@"cpsl"];
    //加工方式id
    [spxscbsDic setValue:@([AD_MANAGER.affrimDic[@"jgfsid"] integerValue]) forKey:@"jgfsid"];
    [spxscbsDic setValue:@([AD_MANAGER.affrimDic[@"jgfdj"] integerValue]) forKey:@"jgfdj"];
    [spxscbsDic setValue:@([colorModel1.saveCount doubleValue] * [AD_MANAGER.affrimDic[@"jgfdj"] integerValue]) forKey:@"jgfje"];

    [spxscbsDic setValue:@"" forKey:@"bz"];
...
//            [spxscbsDic setValue:@([colorModel.savePrice doubleValue]) forKey:@"spdj"];//销售数量
//
//
//            [spxscbsDic setValue:kGet2fDouble([colorModel.savePrice doubleValue] * ([colorModel.saveCount doubleValue] + [colorModel.savekongcha doubleValue])) forKey:@"spje"];//销售金额
//
...
-(void)allOrderCommonParamters{
    ...
    [AD_MANAGER.caoGaoDic setValue:@([AD_MANAGER.affrimDic[@"ckid"] integerValue]) forKey:@"ckid"];//仓库id ?
    
    [AD_MANAGER.caoGaoDic setValue:@([AD_MANAGER.affrimDic[@"jsrid"] integerValue]) forKey:@"jsrid"];//
    
    NSInteger jgcid = [AD_MANAGER.affrimDic[@"gysid"] integerValue];
    [AD_MANAGER.caoGaoDic setValue:@(jgcid) forKey:@"jgcid"];//供应商名称id
    [AD_MANAGER.caoGaoDic setValue:@([AD_MANAGER.affrimDic[@"sfrk"] integerValue]) forKey:@"sfrk"];//是否直发
    
    [AD_MANAGER.caoGaoDic setValue:[self.VC.bzTV.text isEqualToString:@"填写你的备注(选填)"] ? @"" :self.VC.bzTV.text  forKey:@"djbz"];//单据备注
    [AD_MANAGER.caoGaoDic setValue:@([AD_MANAGER.affrimDic[@"jgcid"] integerValue]) forKey:@"gysid"];//供应商名称id
    
    
    [AD_MANAGER.caoGaoDic setValue:@(0)  forKey:@"shrid"];//送货人
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"JDYuanLiaoRuKuTableViewController"]) {
        JDYuanLiaoRuKuTableViewController * nav = [segue destinationViewController];
        self.VC = nav;
...
-(void)yudingdanCommonAction:(NSInteger )autoCheck{
    ...
    [AD_MANAGER.caoGaoDic setValue:[self allOrderYangPinYuDingListCommonParamtersArray] forKey:@"tbnote_jgshcbs"];//商品列表
    ...
    [AD_MANAGER requestJiaGongShouHuoUp:mDic success:^(id object) {
        UIStoryboard * stroryBoard4 = [UIStoryboard storyboardWithName:@"ForthVC" bundle:nil];
        if (autoCheck == 1) {
            [self showToast:[@"加工收货单保存成功" append:object[@"data"]]];
