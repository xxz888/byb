//
//  JDJiaGongFaHuo4ViewController.m
//  UniversalApp
//
...
@interface JDJiaGongFaHuo4ViewController ()<YBPopupMenuDelegate>
@property(nonatomic,strong)JDJiaGongFaHuo4TableViewController * VC;
@property (nonatomic,strong) NSDictionary * _dic1;
@property (weak, nonatomic) IBOutlet UIButton *nextBtn;
@end
@implementation JDJiaGongFaHuo4ViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"加工发货单确认";
}
...
-(void)allOrderCommonParamters{
    ...
    NSInteger jgcid = [AD_MANAGER.affrimDic[@"jgcid"] integerValue];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"JDJiaGongFaHuo4TableViewController"]) {
        JDJiaGongFaHuo4TableViewController * nav = [segue destinationViewController];
        self.VC = nav;
...
-(void)yudingdanCommonAction:(NSInteger )autoCheck{
    ...
    [AD_MANAGER.caoGaoDic setValue:[self allOrderYangPinYuDingListCommonParamtersArray] forKey:@"tbnote_jgfhcbs"];//商品列表
    ...
    [AD_MANAGER requestJiaGongFaHuoUp:mDic success:^(id object) {
        UIStoryboard * stroryBoard4 = [UIStoryboard storyboardWithName:@"ForthVC" bundle:nil];
        if (autoCheck == 1) {
            [self showToast:[@"加工发货单保存成功" append:object[@"data"]]];
