//
//  JDJiaGongFaHuo3ViewController.m
//  UniversalApp
//
...
#import "JDJiaGongFaHuo3ViewController.h"
...
@interface JDJiaGongFaHuo3ViewController ()<UITableViewDelegate,UITableViewDataSource,JDCellPriceChangeDelegate,DissmissVCJiaGongFaHuoDelegate,spVCDissmissVCDelegate,JDYangPinDanChangeDelegate>{
...
@implementation JDJiaGongFaHuo3ViewController
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    kWeakSelf(self);
    //如果是j转厂，就隐藏底部
    if (ORDER_ISEQUAl(JiaGongZhuanChang)) {
        self.hiddenHeignth.constant = 0;
        self.hiddenLbl.hidden = YES;
    }
    
    if(ORDER_ISEQUAl(YuanLiaoRuKu)){
        self.zongjiTagLbl.hidden = NO;
...
- (IBAction)selectOKAction:(id)sender {
    ...
    if (ORDER_ISEQUAl(JiaGongZhuanChang)) {
        ...
        JDJiaGongZhuanChang4ViewController * VC = [stroryBoard instantiateViewControllerWithIdentifier:@"JDJiaGongZhuanChang4ViewController"];
        [self.navigationController pushViewController:VC animated:YES];

    }else if(ORDER_ISEQUAl(YuanLiaoRuKu)){
        UIStoryboard * stroryBoard = [UIStoryboard storyboardWithName:@"YuanLiaoRuKu" bundle:nil];
        JDYuanLiaoRuKuViewController * VC = [stroryBoard instantiateViewControllerWithIdentifier:@"JDYuanLiaoRuKuViewController"];
        [self.navigationController pushViewController:VC animated:YES];
        ...
        JDJiaGongFaHuo4ViewController * VC = [stroryBoard instantiateViewControllerWithIdentifier:@"JDJiaGongFaHuo4ViewController"];
        [self.navigationController pushViewController:VC animated:YES];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
     ...
     //如果是原料入库，就要把单价放开
    if (ORDER_ISEQUAl(YuanLiaoRuKu)) {
        cell.tf1.userInteractionEnabled = YES;
    }else{
        cell.tf1.userInteractionEnabled = NO;
